...
var generateReport = function (options) {
    ...
    var readFileForRespectiveTemplates = function (filename) {
        ...
        return ((options.theme === 'bootstrap2') || (options.theme === 'bootstrap') || (options.theme === 'hierarchy')) ? readFile('../_common/bootstrap.hierarchy/' + filename) : readFile(filename);
    };
    ...
    var setStats = function (suite) {
        ...
        featureOutput.forEach(function (feature) {
            ...
            feature.elements.forEach(function (element) {
                ...
                if(!element.name && options.skipStatsForScenariosStartingWith){
                    let scenario_name = element.name.toString();
                    var scenariosToBeSkipped = options.skipStatsForScenariosStartingWith;    
                    var arrayLength = scenariosToBeSkipped.length;
                    for (var i = 0; i < arrayLength; i++) {
                        if(scenario_name.startsWith(scenariosToBeSkipped[i])){
                            return;
                        }
                    }    
                }
    ...
    fs.writeFileSync(
        ...
        _.template(readFile('index.html'))({
            ...
            piechart: ((options.theme === 'bootstrap2') || (options.theme === 'bootstrap') || (options.theme === 'hierarchy')) ? readFileForRespectiveTemplates('piechart.js') : undefined
        })
    );
